module Api
  class RoutesController < FunctionalitiesController
    ...
    def show
      ...
      if proxy_parameter.authentication_mode != 'null'
        ...
        if proxy_parameter.authentication_mode == 'oauth2'
          ...
          #route_parameter.hostname = 'api.emploi-store.fr'
        end

      end

      uri_str = "#{route_parameter.protocol}://#{route_parameter.hostname}:#{route_parameter.port}/#{route_parameter.url}" #"/?#{URI.encode_www_form(request.query_parameters)}"
      ...
      res = request_server uri_str
      #return render plain: res.inspect
    ...
    def request_server(uri_str, limit = 10)
      ...
      req.add_field 'Authorization', "Bearer #{@access_token}" if @access_token.present?
